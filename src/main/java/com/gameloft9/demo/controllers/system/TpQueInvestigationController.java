...
@Controller
@RequestMapping("/wjdc")
public class TpQueInvestigationController {
    @Resource
    private TpQueInvestigationService investigationQueService;
    ...
    public IResult findAll2(String page,String limit, String qid){
        ...
        return new PageResultBean<Collection<TpQueInvestigation>>(investigationQueService.findAll2(page,limit),investigationQueService.getCount2(qid));
    }
    //添加
    @RequestMapping(value = "/add2.do",method = RequestMethod.POST)
    @ResponseBody
    @BizOperLog(operType = OperType.ADD,memo = "添加")
    public IResult add2(TpQueInvestigation investigationQue){
    ...
    @ResponseBody
    @BizOperLog(operType = OperType.ADD,memo = "添加")
    public IResult addTotal(@RequestBody TpQueTotal totalQue){
        List<TpQueOption> oids = totalQue.getOids();
        String qid = totalQue.getQid();
        for (TpQueOption optionQue : oids) {
            String oid = optionQue.getOid();
            TpQueTotal tot = new TpQueTotal(oid,qid);
