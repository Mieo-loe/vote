...
import com.gameloft9.demo.dataaccess.dao.system.FrameworkMapper;
...
import com.gameloft9.demo.service.api.system.FrameworkService;
...
public class FrameworkServiceImpl implements FrameworkService {
     ...
     * 获取所有组织机构
     */
    public List<OrgNodeResponseQue> getAll() {
        List<FrameworkQue> orgList = new LinkedList<FrameworkQue>();
     ...
     * @param fid 主键
     * */
    public FrameworkQue getById(String fid) {
     ...
     * */
    public Boolean update(FrameworkQue frameworkQue){
     ...
     * @param frameworkQue 组织机构信息
     * */
    public Boolean add(FrameworkQue frameworkQue){
        CheckUtil.notNull(frameworkQue,"组织机构为空");

        //名称不能重复
        FrameworkQue d = frameworkMapper.getByName(frameworkQue.getOrganization());
        CheckUtil.check(d == null,"该组织机构已经存在");

        //根机构只能有一个
        if(StringUtils.isBlank(frameworkQue.getSuperior())){
//            log.info("添加根机构");
            FrameworkQue root = frameworkMapper.getRoot();
...
//        log.info("添加子机构");
        FrameworkQue parent = frameworkMapper.selectByPrimaryKey(frameworkQue.getSuperior());
     ...
     */
    private List<OrgNodeResponseQue> convert2TreeNodes(List<FrameworkQue> orgList) {
        CheckUtil.notNull(orgList, "组织机构列表为空");
        CheckUtil.check(!orgList.isEmpty(), "组织机构列表为空");

        List<OrgNodeResponseQue> res = new LinkedList<OrgNodeResponseQue>();

        Iterator<FrameworkQue> it = orgList.iterator();
        while (it.hasNext()) {
            FrameworkQue d = it.next();
            if (d.getSuperior().equals("0")) {//拿到根机构
                OrgNodeResponseQue node = new OrgNodeResponseQue();
     ...
     * @param orgList  组织机构列表
     */
    private List<OrgNodeResponseQue> getChildrenNodes(String superior, List<FrameworkQue> orgList) {
        List<OrgNodeResponseQue> children = new LinkedList<OrgNodeResponseQue>();

        Iterator<FrameworkQue> it = orgList.iterator();
        while (it.hasNext()) {
            FrameworkQue t = it.next();
