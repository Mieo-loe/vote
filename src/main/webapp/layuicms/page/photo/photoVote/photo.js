layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#photo-data'
            , height: 415
            , url: $tool.getContext() + 'photo/photoList.do' //数据接口(请求地址)
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头 field  返回的实体对象里面属性的数据一致
                  {type:'numbers',title:'序号',fixed: 'left'}
                , {field: 'vid', title: '历史记录', width: '10%'}
                , {field: 'issueTime', title: '发布日期', width: '20%'}
                , {field: 'status', title: '状态', width: '10%',width: '20%'}
