...
layui.config({
    ...
    function checkClick(e) {
        ...
        function initModelfo() {//弹框验证账号
            ...
            $api.GetHisitory_reading(req, function (res) {
                var data = res.data;
                    ...
                    for (var j = 0; j < res.data.list[i].answerList.length; j++) {
                        if (res.data.list[i].optionRule == 78) {
                            zbz += '<div class="answer">\n' +
                                ...
                                '                    </div>'+
                                '<div class="answer_Content">已投票'+res.data.list[i].answerList[j].sumNumber+'</div>';
                        } else {
                            zbz += '             <div class="answer">\n' +
                                '                        <div name="checked' + question_Index + '" class="answer_Checkbox" id="' + res.data.list[i].answerList[j].aid + '"><img width="24" src="../../../page/photo/gou.png" alt=""></div>\n' +
                                '                        <div class="answer_Content">' + res.data.list[i].answerList[j].answer + '</div>\n' +
                                '                    </div>'+
                                '<div class="answer_Content">已投票'+res.data.list[i].answerList[j].sumNumber+'</div>';
                        }
