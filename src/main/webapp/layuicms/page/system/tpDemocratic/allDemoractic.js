layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#role-data'
            , height: 415
            , url: $tool.getContext() + 'demoractic/list.do' //数据接口
            , method: 'post'
            , limit:'5'
            , limits:[5,10,15,20]
            , page:true //开启分页
            , cols: [[ //表头
                  {field:'recordId',title:'序号',width: '20%'},
                  {field: 'bigTitleId', title: '历史记录', width: '20%'}
                , {field: 'releaseDate', title: '发布日期', width: '20%'}
                , {field: 'status', title: '状态', width: '20%', templet:'#fond'}
        ...
        table.on('tool(roleFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //区分事件
            if (layEvent === 'del') { //删除
                delRole(row.recordId);
            } else if (layEvent === 'cancel') { //编辑
                //do something
                updateRole(row.recordId);
            }
        });
    }
    defineTable();
    ...
    function delRole(recordId){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.DeleteDemoractic(req,function (data) {
    ...
    function updateRole(recordId){
        layer.confirm('确认取消吗？', function (confirmIndex) {
            ...
            $api.UpdateDemoractic(req,function (data) {
