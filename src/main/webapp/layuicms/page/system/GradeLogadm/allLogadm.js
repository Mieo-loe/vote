layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            ...
            , id: 'log_id'
            , height: 395
            , url: $tool.getContext() + 'logadm/logList.do' //数据接口
            , method: 'post'
            , page: true //开启分页
            , cols: [[ //表头
                {type: "checkbox"}
                , {field: 'username', title: '操作者', width: '10%'}
                , {field: 'ip', title: 'ip地址', width: '15%'}
                , {field: 'operType', title: '操作类型', width: '10%'}
                // , {field: 'memo', title: '备注', width: '20%'}
                , {field: 'l_time', title: '操作日期', width: '15%'}
    ...
    form.on("submit(queryLog)", function (data) {
        var loginName = data.field.username;
    ...
    function delLogs(log_id) {
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.DeleteLogadm(req,function (data) {
    ...
    function batchDelLog(req) {
        //向服务端发送删除指令
        $api.BatchDeleteLogadm(JSON.stringify(req),{contentType: "application/json;charset=utf-8"},function (data) {
            layer.msg("删除成功", {time: 1000}, function () {
                //obj.del(); //删除对应行（tr）的DOM结构
                //清空列表
                clearParams();
    ...
    //查看所有操作
    $(".editLogs_btn").click(function () {
        var index = layui.layer.open({
            title: "设置记录操作",
            type: 2,
            content: "../../../page/system/GradeLogadm/editLog.html",
