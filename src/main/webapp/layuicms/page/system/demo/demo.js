layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#role-data'
            , height: 415
            , url: $tool.getContext() + 'record/list.do' //数据接口
            , method: 'post'
            , page:true //开启分页
            ,limit:5
            ,limits:[1,2,3,4,5]
...
//删除
    ...
    function editdemo(recordId){
        var a = layer.prompt({title: '请输入账号',formType: 0}, function(content) {
            ...
            $api.jiaoyan(JSON.stringify(req), {contentType: "application/json;charset=UTF-8"}, function (data) {
                alert(data.data)
                if(data.data==false){
                    ...
                    var index = layui.layer.open({
                        title: "预览",
                        type: 2,
                        content: "check.html?recordId="+recordId,
                        success: function (layero, index) {
                            setTimeout(function () {
                                layui.layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                                    tips: 3
