...
form.on('checkbox(type)',function (data) {
    ...
    //预览
    form.on("submit(btn_Yulan)",function (data) {
        var title = $("#title").val();
        ...
        var Yu=JSON.stringify(req);
        window.sessionStorage.setItem("Yu",Yu);
        var index = layui.layer.open({
            title: "预览",
            type: 2,
            content: "addAdmYuLan.html",
    ...
    form.on("submit(keep_btn)", function (data) {
