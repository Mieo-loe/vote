layui.config({
    ...
    function init() {
        ...
        $api.GetAdm(req,function (res) {
            ...
            var users=users.substring(0,users.length-1);
            $("#uname").val(users);
            var uids=uids.substring(0,uids.length-1);
            $("#uid").val(uids);
    ...
    form.on("submit(edit_ready)",function () {
        ...
        window.sessionStorage.setItem("cc",cc);
        var index = layui.layer.open({
            title: "预览",
            type: 2,
            content: "voteUse.html",
    ...
    form.on("submit(addTemplet)",function (data) {
        ...
        //alert(req);//弹框确定是否取到值
        $api.updateTemplet(JSON.stringify(req),{contentType:'application/json;charset=utf-8'},function (data) {
